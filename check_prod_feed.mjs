...
async function testFeed() {
    try {
        ...
        if (posts.length > 0) {
            const devToPosts = posts.filter(p => p.source === 'Dev.to');
            console.log('Dev.to Posts Count:', devToPosts.length);
            if (devToPosts.length > 0) {
                console.log('First Dev.to Post:', devToPosts[0].content.substring(0, 50) + '...');
            }
        } else {
